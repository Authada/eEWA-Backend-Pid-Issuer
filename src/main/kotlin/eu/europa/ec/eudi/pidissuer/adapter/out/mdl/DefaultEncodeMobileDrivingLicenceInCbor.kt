...
private fun MDocBuilder.addItemsToSign(licence: MobileDrivingLicence) {
    ...
    addItemToSign(DrivingPrivilegesAttribute, licence.privileges.map { it.toDataElement() }.toDataElement())
...
private fun DrivingPrivilege.toDataElement() =
    buildMap {
        ...
        restrictions?.let { restrictions ->
            put("codes", restrictions.map { it.toDataElement() }.toDataElement())
        }
    }.toDataElement()

private fun DrivingPrivilege.Restriction.toDataElement() =
    buildMap {
        val (code, sign, value) =
            when (this@toDataElement) {
