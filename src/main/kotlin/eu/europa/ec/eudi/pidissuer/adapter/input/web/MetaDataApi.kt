...
class MetaDataApi(
    ...
    private val getAuthorizationMetaData: GetAuthorizationMetaData,
    ...
    val route = coRouter {
        ...
        GET(WELL_KNOWN_OAUTH_CONFIGURATION, accept(MediaType.APPLICATION_JSON)) { _ ->
            handleGetAuthroizationMetaData()
        }
    ...
    private suspend fun handleGetAuthroizationMetaData(): ServerResponse =
        getAuthorizationMetaData().let { metaData -> ServerResponse.ok().json().bodyValueAndAwait(metaData.toMap()) }

    private suspend fun handleGetJwtIssuerJwkSet(): ServerResponse = ServerResponse.ok().json()
        .bodyValueAndAwait(Json.parseToJsonElement(credentialIssuerMetaData.jwtVcIssuerJwks.toString(true)))
    ...
    private suspend fun handleGetJwtVcIssuerMetadata(): ServerResponse =
        ServerResponse.ok()
            ...
            .bodyValueAndAwait(
                buildJsonObject {
                    put("issuer ", JsonPrimitive(credentialIssuerMetaData.id))
    ...
    companion object {
        ...
        const val WELL_KNOWN_OAUTH_CONFIGURATION = "/.well-known/oauth-authorization-server"
