...
// verifier key agreement parameter for authenticated channel
...
fun CredentialRequest.assertIsSupported(meta: CredentialConfiguration) =
    when (this) {
        ...
        is SeTlvVcCredentialRequest -> {
            ensure(meta is SeTlvVcCredentialConfiguration) { "Was expecting a ${SE_TLV_FORMAT.value}" }
            validate(meta)
        }
    }
